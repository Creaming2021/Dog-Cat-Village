...
contract Lottery {
    ...
    bool private mode = false; // false: answerForTest, true: real block hash
    ...
    function distribute() public {
        // head 3 4 5 7 8 9 10 11 12 tail
        uint256 cur;
    ...
    function transferAfterPayingFee(address payable addr, uint256 amount) internal returns (uint256) {
        ...
        // smartcontract 에서 이더를 전송하는 방법 3가지
        // call, send, transfer 
        // transfer: 돈을 보내는데 실패하면 트랜잭션을 무효로 한다
        // send: 실패시 catch로 체크할 수 있다 (실패 경우를 생각못하면 위험)
        // call: function을 호출할 때 이더를 보낼 수 있다 (외부의 스마트컨트랙트를 호출할 때 위험)
    ...
    function pushBet(byte challenges) internal returns (bool) {
        BetInfo memory b;
        b.bettor = msg.sender; // 20bype => 
        b.answerBlockNumber = block.number + BET_BLOCK_INTERVAL; // 32bype => 20000 gas
